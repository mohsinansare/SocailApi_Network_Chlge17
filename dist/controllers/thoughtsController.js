...
export const createthoughts = async (req, res) => {
    try {
        ...
        const user = await User.findOneAndUpdate({ _id: req.body.userId }, { $addToSet: { thoughts: newthoughts._id } }, { new: true });
        if (!user) {
...
export const updatethoughts = async (req, res) => {
    try {
        const updatedthoughts = await thoughts.findOneAndUpdate({ _id: req.params.thoughtsId }, { $set: req.body }, { runValidators: true, new: true });
        if (!updatedthoughts) {
...
export const deletethoughts = async (req, res) => {
    try {
        ...
        const user = await User.findOneAndUpdate({ thoughts: req.params.thoughtsId }, { $pull: { thoughts: req.params.thoughtsId } }, { new: true });
        if (!user) {
            return res
                .status(404)
...
export const addthoughtsResponse = async (req, res) => {
    try {
        const updatedthoughts = await thoughts.findOneAndUpdate({ _id: req.params.thoughtsId }, { $addToSet: { responses: req.body } }, { runValidators: true, new: true });
        if (!updatedthoughts) {
            return res.status(404).json({ message: 'No thoughts with this id!' });
        }
        res.json(updatedthoughts);
...
export const removethoughtsResponse = async (req, res) => {
    try {
        const updatedthoughts = await thoughts.findOneAndUpdate({ _id: req.params.thoughtsId }, { $pull: { reactions: { responseId: req.params.responseId } } }, { runValidators: true, new: true });
        if (!updatedthoughts) {
            return res.status(404).json({ message: 'No thoughts with this id!' });
        }
        res.json(updatedthoughts);
